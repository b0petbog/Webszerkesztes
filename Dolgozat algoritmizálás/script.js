...
function removeFirstAndLast(string) 
{
    ...
    {
        if(string=="" || lista.length==1  || lista.length==2)
        {
            break;
        }
        else if(i!=1 || i!=lista.length)
        {
            uj+=lista[i-1];
        }
    }
...
{
    let emojiTomb = emojiString.split(" ");
    let szamok = "";
    if(emojiString!="")
    {
        for(let k=0;k<emojiTomb.length;k++)
        {
            if(emojiTomb[k]==":)")
            {
                szamok+="0";
            }
            else if(emojiTomb[k]==":D")
            {
                szamok+="1";
            }
            else if(emojiTomb[k]==">(")
